...
public class DungeonRoomSpawner extends DungeonRoom {
	int sizeX;
	int sizeY;
	int sizeZ;
	Random rand;
	...
	public DungeonRoomSpawner(DungeonMapGen dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir) {
	...
	protected DungeonRoom makeRoom(DungeonMapGen dungeon, int x, int y, int z, ForgeDirection dir) {
		return new DungeonRoomSpawner(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand) {
		for (final ChunkCoordinates spawnerCoords : this.spawners) {
			if (this.worldObj.getBlock(spawnerCoords.posX, spawnerCoords.posY, spawnerCoords.posZ) == Blocks.mob_spawner) {
				...
				if (spawner != null) {
					spawner.func_145881_a().setEntityName(DungeonRoomSpawner.getMob(rand));
				}
			}
		}
	}
