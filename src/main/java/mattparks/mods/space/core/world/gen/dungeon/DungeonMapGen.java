...
public class DungeonMapGen {
	...
	public final int hallwayLength;
	public final int hallwayHeight;
	...
	public DungeonMapGen(GenerationSettings settings, int range, int hallwayLength, int hallwayHeight) {
	...
	public void generate(World world, Random rand, int x, int y, int z, int chunkX, int chunkZ, Block[] blocks, byte[] metas, boolean useArrays) {
		DungeonMapGen.useArrays = useArrays;
		this.worldObj = world;
		...
		for (int i = 0; i <= length; i++) {
			for (int j = 0; j < 8; j++) {
				...
				{
				case EAST: // East z++
					offsetZ = this.hallwayLength + rand.nextInt(15);
					if (rand.nextBoolean()) {
						if (rand.nextBoolean()) {
							entranceDir = ForgeDirection.NORTH;
							offsetX = this.hallwayLength + rand.nextInt(15);
						} else {
	...
	private void genCorridor(DungeonBoundingBox corridor, Random rand, int y, int cx, int cz, ForgeDirection dir, Block[] blocks, byte[] metas, boolean doubleCorridor) {
		for (int i = corridor.minX - 1; i <= corridor.maxX + 1; i++) {
			for (int k = corridor.minZ - 1; k <= corridor.maxZ + 1; k++) {
				loopj: for (int j = y - 1; j <= y + this.hallwayHeight; j++) {
					boolean flag = false;
					int flag2 = -1;
					...
					case EAST:
						...
						if (i == corridor.minX - 1 || i == corridor.maxX + 1 || j == y - 1 || j == y + this.hallwayHeight) {
							flag = true;
						}
					...
					case NORTH:
						...
						if (k == corridor.minZ - 1 || k == corridor.maxZ + 1 || j == y - 1 || j == y + this.hallwayHeight) {
							flag = true;
						}
					...
					if (!flag) {
						...
						this.placeBlock(blocks, metas, i, j, k, cx, cz, settings.blockBrick.getFirst(), settings.blockBrick.getSecond());
					}
				}
			}
		}
		...
		for (int i = x - range; i < x + range; i++) {
			for (int k = z - range; k < z + range; k++) {
				...
				for (int j = maxLevel + 3; j > 0; j--) {
					if ((Blocks.air != this.getBlock(blocks, i, j - 1, k, cx, cz) || this.getBlock(blocks, i, j, k, cx, cz) == settings.blockBrick.getFirst()) && helper <= depth) {
